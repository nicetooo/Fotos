...
pub fn platform_webview_version() -> Result<String> {
  // On iOS (both device and simulator), NSBundle::bundleWithIdentifier("com.apple.WebKit") 
  // can crash in CoreFoundation's _CFBundleGetBundleWithIdentifier due to a system bug,
  // especially on iOS 26.x simulators. Since WebKit is always available on iOS and the
  // version check is only informational, we return a placeholder on iOS.
  #[cfg(target_os = "ios")]
  {
    // WebKit is built into iOS, so it's always available.
    // Return a generic version string to avoid the crash.
    return Ok("Built-in".to_string());
  }
